  ...
  if (isUnlocked) {
    return (
      <div className={`flex h-screen overflow-hidden font-sans relative transition-colors duration-500 ${theme === "dark" ? "bg-[#0f1117] text-white" : "bg-[#f8fafc] text-gray-900"}`}>
        ...
        {/* ─── Add New Contact Modal (root level) ─── */}
        {showAddContactModal && (
          <div
            className="fixed inset-0 flex items-center justify-center z-[500] p-4"
            style={{ backgroundColor: "rgba(15,23,42,0.72)", backdropFilter: "blur(8px)", animation: "emojiPickerIn 0.2s cubic-bezier(0.34,1.4,0.64,1)" }}
          ...
          >
            <div className="w-full max-w-3xl rounded-3xl overflow-hidden" style={{ boxShadow: "0 30px 80px rgba(99,102,241,0.3), 0 8px 32px rgba(0,0,0,0.2)" }} onClick={(e) => e.stopPropagation()}>
              <div className="px-8 py-5 flex justify-between items-center relative overflow-hidden" style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }}>
                ...
                <div className="flex rounded-2xl p-1 mx-auto" style={{ background: "rgba(255,255,255,0.15)" }}>
                  {[{ key: "direct", label: "Direct Message", icon: <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
                    ...
                    <button key={key} onClick={() => setNewContactType(key)} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold transition-all ${newContactType === key ? "bg-white text-violet-700 shadow-md" : "text-white/70 hover:text-white"}`}>{icon}{label}</button>
                  ))}
                </div>
              ...
              <div className={`grid grid-cols-2 divide-x ${theme === "dark" ? "bg-[#1a1f2e] divide-gray-700/60" : "bg-white divide-gray-100"}`}>
                <div className="p-6 flex flex-col gap-4">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Alice Johnson" autoFocus value={newContactName} onChange={(e) => setNewContactName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddContact()} className={`w-full px-4 py-3 rounded-2xl text-sm font-medium outline-none border transition-colors ${theme === "dark" ? "bg-gray-800/60 border-gray-700 text-white placeholder:text-gray-500 focus:border-violet-500" : "bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-400 focus:border-violet-400"}`} />
                  </div>
                  {newContactType === "direct" ? (
                    <div>
                      ...
                      <input type="text" placeholder="e.g. Online • Secure" value={newContactPhone} onChange={(e) => setNewContactPhone(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAddContact()} className={`w-full px-4 py-3 rounded-2xl text-sm font-medium outline-none border transition-colors ${theme === "dark" ? "bg-gray-800/60 border-gray-700 text-white placeholder:text-gray-500 focus:border-violet-500" : "bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-400 focus:border-violet-400"}`} />
                    </div>
                  ) : (
                    <div>
                      ...
                      <input type="text" placeholder="What's this group about?" value={newGroupDescription} onChange={(e) => setNewGroupDescription(e.target.value)} className={`w-full px-4 py-3 rounded-2xl text-sm font-medium outline-none border transition-colors ${theme === "dark" ? "bg-gray-800/60 border-gray-700 text-white placeholder:text-gray-500 focus:border-violet-500" : "bg-gray-50 border-gray-200 text-gray-900 placeholder:text-gray-400 focus:border-violet-400"}`} />
                    </div>
                  )}
                  <div className="mt-auto">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {CONTACT_COLORS.map((color) => (
                        <button key={color} onClick={() => setNewContactColor(color)} className={`w-8 h-8 rounded-xl ${color} transition-all hover:scale-110 active:scale-95 flex items-center justify-center`} style={{ boxShadow: newContactColor === color ? "0 0 0 3px white, 0 0 0 5px #8b5cf6" : "none" }}>
                          {newContactColor === color && <svg viewBox="0 0 24 24" width="14" height="14" fill="white"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {newContactType === "group" ? (
                    <>
                      <label className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>
                        Members *
                        <span className={`text-[10px] font-semibold normal-case tracking-normal px-1.5 py-0.5 rounded-full ${newGroupInitialMembers.length < 2 ? theme === "dark" ? "bg-red-500/20 text-red-400" : "bg-red-50 text-red-500" : theme === "dark" ? "bg-emerald-500/20 text-emerald-400" : "bg-emerald-50 text-emerald-600"}`}>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {newGroupInitialMembers.map((m) => (
                            <span key={m.id} className={`flex items-center gap-1.5 pl-1 pr-2 py-1 rounded-full text-xs font-bold ${theme === "dark" ? "bg-gray-700 text-gray-200" : "bg-violet-50 text-violet-800"}`}>
                              ...
                              {m.name}
                              <button onClick={() => setNewGroupInitialMembers((prev) => prev.filter((x) => x.id !== m.id))} className={`ml-0.5 w-3.5 h-3.5 rounded-full flex items-center justify-center transition-colors ${theme === "dark" ? "hover:bg-red-500/30 text-gray-400 hover:text-red-400" : "hover:bg-red-100 text-violet-400 hover:text-red-500"}`}>
                      ...
                      <div className={`flex gap-2 p-1.5 rounded-2xl border ${theme === "dark" ? "bg-gray-800/60 border-gray-700" : "bg-gray-50 border-gray-200"}`}>
                        <input type="text" value={newGroupMemberInput} onChange={(e) => setNewGroupMemberInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && newGroupMemberInput.trim()) { const inputName = newGroupMemberInput.trim(); const existing = contacts.find((c) => c.type === "direct" && c.name.toLowerCase() === inputName.toLowerCase()); const alreadyAdded = newGroupInitialMembers.some((m) => m.name.toLowerCase() === inputName.toLowerCase()); if (alreadyAdded) return; const colors = ["bg-violet-500","bg-blue-500","bg-emerald-500","bg-pink-500","bg-amber-500","bg-cyan-500","bg-rose-500","bg-indigo-500"]; const entry = existing ? { id: existing.id, name: existing.name, color: existing.color } : { id: `new-${Date.now()}`, name: inputName, color: colors[Math.floor(Math.random() * colors.length)] }; setNewGroupInitialMembers((prev) => [...prev, entry]); setNewGroupMemberInput(""); } }} placeholder="Add by name — existing or new…" className={`flex-1 bg-transparent text-sm px-2 outline-none ${theme === "dark" ? "text-white placeholder-gray-600" : "text-gray-900 placeholder-gray-400"}`} />
                        <button onClick={() => { const inputName = newGroupMemberInput.trim(); if (!inputName) return; const existing = contacts.find((c) => c.type === "direct" && c.name.toLowerCase() === inputName.toLowerCase()); const alreadyAdded = newGroupInitialMembers.some((m) => m.name.toLowerCase() === inputName.toLowerCase()); if (alreadyAdded) return; const colors = ["bg-violet-500","bg-blue-500","bg-emerald-500","bg-pink-500","bg-amber-500","bg-cyan-500","bg-rose-500","bg-indigo-500"]; const entry = existing ? { id: existing.id, name: existing.name, color: existing.color } : { id: `new-${Date.now()}`, name: inputName, color: colors[Math.floor(Math.random() * colors.length)] }; setNewGroupInitialMembers((prev) => [...prev, entry]); setNewGroupMemberInput(""); }} disabled={!newGroupMemberInput.trim()} className="px-3 py-1.5 rounded-xl text-xs font-bold text-white flex-shrink-0 transition-all hover:scale-105 disabled:opacity-40" style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)" }}>
                          + Add
                        </button>
                      </div>
                      {(() => { const suggestions = contacts.filter((c) => c.type === "direct" && !newGroupInitialMembers.some((m) => m.id === c.id) && (newGroupMemberInput === "" || c.name.toLowerCase().includes(newGroupMemberInput.toLowerCase()))); if (suggestions.length === 0) return null; return (<div className={`rounded-2xl border overflow-hidden ${theme === "dark" ? "border-gray-700 bg-gray-800/60" : "border-gray-200 bg-white"}`}>{suggestions.map((c) => (<button key={c.id} onClick={() => { setNewGroupInitialMembers((prev) => [...prev, { id: c.id, name: c.name, color: c.color }]); setNewGroupMemberInput(""); }} className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-colors ${theme === "dark" ? "hover:bg-gray-700/60" : "hover:bg-violet-50"}`}><div className={`w-6 h-6 rounded-lg ${c.color} flex items-center justify-center flex-shrink-0`}><span className="text-white font-black" style={{ fontSize: "9px" }}>{c.name.charAt(0)}</span></div><span className={`text-xs font-semibold ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>{c.name}</span></button>))}</div>); })()}
                    </>
                  ) : (
              ...
              <div className={`px-8 py-4 flex gap-3 border-t ${theme === "dark" ? "bg-[#111827] border-gray-800" : "bg-gray-50 border-gray-100"}`}>
                <button onClick={() => setShowAddContactModal(false)} className={`flex-1 py-2.5 rounded-2xl text-sm font-bold transition-all active:scale-95 ${theme === "dark" ? "bg-gray-800 text-gray-300 hover:bg-gray-700" : "bg-gray-200 text-gray-600 hover:bg-gray-300"}`}>Cancel</button>
                <button onClick={handleAddContact} disabled={!newContactName.trim() || (newContactType === "group" && newGroupInitialMembers.length < 2)} className="flex-1 py-2.5 rounded-2xl text-sm font-bold text-white transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed" style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }}>
